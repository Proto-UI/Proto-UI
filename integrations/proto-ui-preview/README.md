...
The security workflow checks out the pinned contract repository to verify its real Go handler sources and run its preview test suite. Pull-request runs attempt that checkout only with the ordinary `GITHUB_TOKEN` and otherwise run the in-repository contract tests; the credentialed pinned verification lives in a separate job that runs solely from immutable default-branch (`push`) workflow code, so a pull request can never receive the optional `DCBOT_CONTRACT_TOKEN` secret. Because `Proto-UI/dcbot` is private, both checkouts are marked `continue-on-error`: when the token is absent or unauthorized, the jobs stay green and skip only the pinned-source digest verification and the Go handler suite. Configure `DCBOT_CONTRACT_TOKEN` with a fine-grained read-only token for `Proto-UI/dcbot` to restore full pinned-source verification on every default-branch push.
