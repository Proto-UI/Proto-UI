...
The security workflow verifies the pinned contract repository's real Go handler sources and runs its preview test suite. The `pull_request` lane executes contributor-controlled code, so it stays secret-free and never contacts the private `Proto-UI/dcbot` repository. Authoritative pull request evidence comes from the `pull_request_target` lane, which runs from the immutable default-branch workflow definition with a read-only `GITHUB_TOKEN`: it requires `DCBOT_CONTRACT_TOKEN`, uses that credential only to check out the pinned `Proto-UI/dcbot` revision (with `persist-credentials: false` and a verified resolved commit), reads the exact pull request head's contract JSON as inert data without executing anything from that checkout, verifies the pinned source digests with a script embedded in the trusted workflow itself, and runs the pinned handler's Go suite. Its check is bound to the exact pull request head SHA, and a missing credential, an unreachable pinned revision, or a digest mismatch fails the check instead of skipping it, so exact-head verification is non-optional before integration. The same fail-closed pinned verification runs on every default-branch `push`. Configure `DCBOT_CONTRACT_TOKEN` with a fine-grained read-only token for `Proto-UI/dcbot`.

Fallback jobs serialize on the API-derived PR number and recheck the live open PR plus exact head after acquiring the lock. The resolver's artifact size is rejected at the fallback receiver's 50 MiB compressed envelope before anything is downloaded. The fallback then fetches the verified artifact through the GitHub API with a streaming compressed cap and extracts it with a bounded reader that enforces the file-count, per-file, expanded, symlink/special-file, and path-safety limits from the ZIP central directory before any byte is inflated or materialized; the sanitizer re-enforces the same envelope on the resulting tree before archiving. Immediately before the Ready write, the job revalidates the live open PR and exact head so a stale artifact converges to Failed instead of Ready. A live attempt that does not reach Ready makes a best-effort Failed transition for the same tuple, writes a failed sticky state, and leaves the job failed; this includes an ambiguous Building response. The comment writer repeats the live state/head check immediately before mutation. A rejected Closed transition fails cleanup even while Cloudflare deletion is disabled. Because the owning control plane cannot be derived from the current kill-switch value, close and fallback-unavailable flows revoke the current head on the central Poppy plane always and on the configured fallback plane whenever one exists.
