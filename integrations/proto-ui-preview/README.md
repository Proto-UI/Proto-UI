...
The dcbot fallback is available only when the repository Actions variable `POPPY_PREVIEW_FALLBACK_ORIGIN` is a configured HTTPS origin and `POPPY_CLOUDFLARE_MUTATIONS_ENABLED` is not `true`. The trusted workflow uploads a sanitized `tar.gz` to `POST /api/preview/deployments` with `X-Poppy-Signature-256: sha256=<HMAC(raw tar.gz)>`, `X-Poppy-Preview-PR`, `X-Poppy-Preview-Head-SHA`, `X-Poppy-Preview-Run-ID`, and `X-Poppy-Preview-Run-Attempt`. Poppy serves the private artifact at `/admin/preview/content/<pr>/<head-sha>/...`, using its existing OAuth/session authorization, `private, no-store`, CSP, exact-head replacement, and close cleanup. When the origin variable is absent, the workflow emits **Preview unavailable** instead of claiming Ready.
