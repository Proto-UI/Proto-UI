...
export function renderProtoStyleTokenCss(tokens: string[]): string {
  ...
  // be emitted after them. Text-size utilities also carry a default line-height,
  // so explicit `leading-*` overrides must be emitted after every `text-*` rule.
  // The incoming token list is alphabetical, which would otherwise let those
  // defaults win the cascade. Stable-sort both override groups to the end.
  const utilityName = (rule: CssRule): string => splitVariants(rule.token).at(-1) ?? '';
  const timingOverride = (rule: CssRule): boolean =>
    /^(duration|ease|delay)-/.test(utilityName(rule));
  const leadingOverride = (rule: CssRule): boolean => utilityName(rule).startsWith('leading-');
  const cascadePriority = (rule: CssRule): number => {
    if (timingOverride(rule)) return 2;
    if (leadingOverride(rule)) return 1;
    return 0;
  };
  rules.sort((a, b) => cascadePriority(a) - cascadePriority(b));
