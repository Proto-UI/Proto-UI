...
| Proto UI Terminal semantic owner | One logical surface identity; attachment/display facts; resolved input mode; committed text/key intent; resize correlation; bounded attention/error; lease identity and stale-result rejection. | Plain immutable facts/requests plus Focus-domain identity, view epoch, and request/fact participation; Focus remains sole fact owner. | Cell/grid/scrollback, raw bytes, host events/geometry, engine modes/controllers, focus fact ownership, App credentials or process authority. |
