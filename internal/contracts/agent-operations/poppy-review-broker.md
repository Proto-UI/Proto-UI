...
All writes carry `commit_id` equal to the reviewed head. The `proto-ui-scheduled-review-v1` authorization is schedule and local-runner bound, so it does not by itself authorize the event-driven Poppy reviewer; a separately recorded Poppy authorization keyed to Poppy's own event-admission execution source must be active before any Poppy review write. That Poppy authorization must prohibit `APPROVE` whenever any current or previous changed-file path identifies a YAML entity under the governed `spec/**` collections; widening that path gate requires another separately recorded authorization and matching repository-side enforcement before activation.
...
| P1 event-driven shadow | Reported `(1,0,0,0)`; pending dated rollout record | Continue collecting stale/replay/fork/concurrency/permission-loss observations with review writes disabled and no `Actions: write` expansion. A dated `internal/records/*` entry must bind the `(1,0,0,0)` shadow state to the deployed dcbot revision, effective token and permission topology, switch tuple, shadow receipt, and observation window before this row is treated as evidenced. |
