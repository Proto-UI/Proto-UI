...
humanDecisions:
  ...
  integration:
    status: pending
    decision: No commit, merge, release, or change-grouping decision has been made.
...
decisionRequested:
  - Decide separately whether and how the technically completed remediation should be committed, merged, or released.
...
Round one classified the broad packet as `incomplete` with confidence `0.99` and recommended `revise`. Round two independently classified the narrowed packet as `adequate` with confidence `0.96`; it found no material packet correction and recommended gathering Node 22 evidence before approval.
...
- [x] Re-run the final validation on Node 22 before merge approval.
- [ ] Record the separate human integration decision when commit, merge, or release is requested.
