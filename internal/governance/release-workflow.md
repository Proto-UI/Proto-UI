...
npm Trusted Publisher configuration is package-scoped and therefore cannot be attached before a package identity exists. Every newly named public package must be created before the release train with a clearly non-release bootstrap version and configured for the reviewed release workflow. Bootstrap must not receive a release-channel dist-tag and its `bootstrap` tag must be removed after identity setup. npm may refuse to remove `latest` when it points at the package's sole bootstrap version; that deprecated `latest` may remain after a real prerelease is published only when `next` resolves to an existing non-bootstrap prerelease version. No release-train or stable version may be published merely to cover the bootstrap tag. `pnpm release:registry:check` verifies these public identity, dist-tag, and deprecation conditions; it does not claim to inspect the private Trusted Publisher configuration.
