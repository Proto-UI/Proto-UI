...
The active local `proto-ui` schedule reviews `Proto-UI/Proto-UI` pull requests under `proto-ui-scheduled-review-v1`. It submits complete, finding-backed `REQUEST_CHANGES` or a clean `APPROVE` disposition when the content-specific review class, canonical v4 input, reviewer identity distinct from the PR author and every commit contributor, trusted CI, trusted DCO status, live permission, and repository rules agree. A path under `spec/**` is not a blanket gate: governed refinements and evidence repairs proceed, while genuinely unresolved product direction produces the one decision packet.

`proto-ui-scheduled-merge-v1` then squash-merges a clean exact head after platform-contributor-independent approval, no active `CHANGES_REQUESTED`, resolved review threads, separately trusted CI and DCO status, live permission, and GitHub `MERGEABLE`/`CLEAN` state agree. Commit grouping, review disposition, and merge are evidence-driven transitions under this standing scope, not repeated maintainer prompts.
