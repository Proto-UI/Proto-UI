...
- `scripts/agent-operations/collect-live-collaboration-state.mjs`: live GitHub collection and the seven exact collaboration mutation primitives, each limited to one write and one post-write verification; unknown outcomes receive one reconciliation and no blind retry.
...
Run `agent:collaborate -- request-digest` after the request is complete, then put `sha256:<requestDigest>` on the handoff's `collaboration-request` artifact and the exact authorization ID on its `mutation-authorization` artifact. Autonomous handoffs also bind the loaded self-assessment through the `capability-envelope` digest. `apply` rejects a stale or mismatched preflight before writing, emits a zero-write receipt when the desired state is already live, or performs one write followed by one verification read. Only the unique bounded-comment marker can prove an otherwise unknown write; every other unknown outcome stops after one reconciliation and says not to retry blindly.
