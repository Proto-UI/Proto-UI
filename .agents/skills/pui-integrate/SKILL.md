...
4. Require the target base fixed by the authorization, successful trusted repository CI, no active exact-head `CHANGES_REQUESTED`, every review thread resolved, and at least one exact-head approval from a reviewer other than the pull-request author.
...
8. Treat `allowed: false` as a terminal no-write result for this input. After an unknown write outcome, accept only the command's single live reconciliation; never retry blindly.
