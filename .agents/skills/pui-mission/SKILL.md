...
1. Require a current capability envelope, one candidate selected by a maintainer, explicit authorization for the queue and mission paths, and a fresh probe for `update-tracked-maintenance-state`.
...
5. Create the mission packet and synchronize only the queue fields covered by the authorization and probe.
