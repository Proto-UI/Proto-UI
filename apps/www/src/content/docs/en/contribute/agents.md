...
Local review is always available. A low-band Agent working with you may return a partial review or `ABSTAIN` with clear limitations. Submitting that review to GitHub is a separate human-assisted action and requires your authorization plus a credential with permission; an autonomous run stops first. The final preflight re-collects the whole canonical input (body, commits, replies, threads, checks) live from GitHub and rejects a packet whose digest no longer matches; it also derives the viewer identity, pull-request author, and credential permission from that live context instead of trusting caller-provided strings, and rejects a packet made stale by an intervening push.
