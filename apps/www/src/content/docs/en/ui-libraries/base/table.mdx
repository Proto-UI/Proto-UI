...
Base Table is a **draft protocol with Checkpoint B host-neutral implementation**, not a released Adapter conformance claim. `D-BASE-TABLE-STRUCTURE-0001`, `C-TABLE-STRUCTURE-0001`, `M-TABLE-STRUCTURE-0001`, the five `P-BASE-TABLE*` identities, and `T-TABLE-STRUCTURE-0001` govern the current slice. Four-Adapter profile evidence, CLI facade registration, usable runtime previews, and design-language work remain planned under #621 Checkpoint C.
...
The definition now has a portable Module and Base family; it still does not claim that an official Adapter implements the complete host-projection path.
...
The generic A11y path now supports opaque semantic-object references and ordered multi-target relationships through the merged #625 prerequisite. Table-specific mapping by each official Adapter profile remains planned and is not inferred from that generic transport.
...
The host-neutral reference probe and Checkpoint B Module/Base tests now cover deterministic placement, multiple-header resolution, diagnostics, nearest-domain isolation, authored-fact churn, exact anatomy, and passive negative boundaries. The new `@proto.ui/module-table-structure` and `@proto.ui/prototypes-base/table` exports remain draft. `T-TABLE-STRUCTURE-0001` keeps all four official Web Adapter paths in `planned` status until Checkpoint C adds and independently reviews those tests; this page therefore provides no interactive Table preview yet.
