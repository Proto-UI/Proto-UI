...
In the 0.3 implementation, Trigger and Content pair by exact `value` within their Anatomy domain. Keys such as `a+b` and `a b` remain distinct. Missing or duplicate counterparts leave the corresponding relationship unprojected. Web adapters own the host IDs, preserve authored IDs and independent IDREF tokens, withdraw the pair while either endpoint is detached, and restore a valid reserved identity on rematerialization. Applications should use protocol values rather than depend on a generated ID format.
