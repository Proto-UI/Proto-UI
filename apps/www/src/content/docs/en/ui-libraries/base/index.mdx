...
Dropdown-menu does not, at this layer, define persistent selection, checked/radio item semantics, or submenu anatomy. The transient active navigation state is kept internal for now instead of being committed as root API. Selection-oriented primitives such as `select` and `radio-group` are modeled as separate base families instead of being folded into menu items.
