...
| Skill         | 完成的一次转换                                 |
| ------------- | ---------------------------------------------- |
| `pui-assess`  | 从未测评状态得到未签名的 U0/C1 理解结果        |
| `pui-orient`  | 从未知上下文得到有效能力边界                   |
...
C1 适合有边界的只读事实工作。C2 适合受治理、可逆的维护。C3 适合已批准的语义实现和独立复核。C4 适合复杂语义、跨域协调和发布准备。准确规则以机器 policy 为准。

档位只是一个条件。GitHub 实时权限、与任务有关时的 Discord 或 Poppy 信任、任务风险、准确的 probe、当前人工授权和必须由人完成的闸门，都会继续缩小动作范围。任何一项都不能抬高另一项。
