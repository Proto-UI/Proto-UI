...
Vue 3 使用 `createVueAdapter(Vue)(imageRoot)`，Vue 2 使用 `createVue2Adapter({ extend: Vue.extend.bind(Vue), nextTick: Vue.nextTick.bind(Vue), set: Vue.set.bind(Vue) })(imageRoot)`。Web Components 使用 `AdaptToWebComponent(imageRoot, { registerAs: 'app-image' })`，挂载前通过 `setElementProps()` 设置声明的 props。应用内各个包应使用一致的精确版本。
