...
在 0.3 实现中，Trigger 与 Content 在同一 Anatomy domain 内按精确 `value` 配对，`a+b` 与 `a b` 等 key 保持区别。缺失或重复 counterpart 时，对应关系不投影。Web Adapter 拥有宿主 ID，保留作者 ID 与独立的 IDREF token；任一端 detach 时撤销该关系，重新物化时恢复有效的 identity reservation。应用应使用 protocol value，不应依赖生成 ID 的格式。
